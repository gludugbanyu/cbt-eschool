...
exit;
    ...
    if ($tipe_soal == 'Pilihan Ganda' || $tipe_soal == 'Pilihan Ganda Kompleks') {
        $p1 = mysqli_real_escape_string($koneksi,
            bersihkan_html(
                sanitize_summernote($_POST['pilihan_1'] ?? '')
            )
        );

        $p2 = mysqli_real_escape_string($koneksi,
            bersihkan_html(
                sanitize_summernote($_POST['pilihan_2'] ?? '')
            )
        );

        $p3 = mysqli_real_escape_string($koneksi,
            bersihkan_html(
                sanitize_summernote($_POST['pilihan_3'] ?? '')
            )
        );

        $p4 = mysqli_real_escape_string($koneksi,
            bersihkan_html(
                sanitize_summernote($_POST['pilihan_4'] ?? '')
            )
        );

        $p5 = mysqli_real_escape_string($koneksi,
            bersihkan_html(
                sanitize_summernote($_POST['pilihan_5'] ?? '')
            )
        );
    ...
    bersihkan_html(
        ...
        foreach ($_POST['pasangan_soal'] as $i => $soal) {
            ...
            if (!empty($soal) && !empty($jawaban)) {
                $pasangan_data[] =
mysqli_real_escape_string($koneksi,
    bersihkan_html(
        sanitize_summernote(trim($soal))
    )
)
. ":" .
mysqli_real_escape_string($koneksi,
    bersihkan_html(
        sanitize_summernote(trim($jawaban))
    )
);
        ...
        $jawaban_benar = mysqli_real_escape_string($koneksi,
    bersihkan_html(
                            ...
                            <form method="POST" id="formSoal">
                                <div class="row">
                                    <div class="col-md-2 mb-3">
                                        ...
                                        <input type="number" class="form-control" name="nomer_soal"
                                            value="<?= $nomer_baru ?>" required>
                                    </div>
                                    <div class="col-md-4 mb-3">
                                        <label for="tipe_soal" class="form-label">Tipe Soal</label>
                                        <select class="form-control" id="tipe_soal" name="tipe_soal"
                                            onchange="showFields(this.value)" required>
                                ...
                                <div id="bs-fields" class="d-none">
                                    <div id="bs-container">
                                        <div class="border-box bs-row" id="bs_row_1">
                                            ...
                                            <div class="mt-2">
                                                <label><input type="radio" name="jawaban_benar[0]" value="Benar">
                                                    Benar</label>
                                                <label class="ms-3"><input type="radio" name="jawaban_benar[0]"
                                                        value="Salah"> Salah</label>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div id="match-fields" class="d-none">
                                    <div id="match-container">
                                        <div class="row mb-2 match-row">
                                            <div class="col-md-5"><textarea class="form-control" name="pasangan_soal[]"
                                                    placeholder="Pilihan"></textarea></div>
                                            <div class="col-md-5"><textarea class="form-control"
                                                    name="pasangan_jawaban[]" placeholder="Pasangan"></textarea></div>
